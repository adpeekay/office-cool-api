...
# ======================================================
# Representative EPW per country (explicit & honest)
# ======================================================
...
class CoolingRequest(BaseModel):
    lat: float = Field(
        ...
        ge=-90,
        le=90,
        description="Latitude (degrees) – legacy / fallback only"
    )
    lon: float = Field(
        0.1218,
        ge=-180,
        le=180,
        description="Longitude (degrees) – legacy / fallback only"
    )
...
def calculate_cooling(req: CoolingRequest):
    ...
    if req.iso3 and req.iso3 in COUNTRY_EPW:
        ...
        # Backwards‑compatible fallback (map-based)
        epw_path = get_epw(req.lat, req.lon)
        _, meta = read_epw(epw_path)
        if meta.get("country") not in (None, "unknown"):
            iso3 = meta.get("country")

    # --------------------------------------------------
    # 2. Resolve country data
    # --------------------------------------------------

    country_row = COUNTRY_DATA.get(iso3)

    country_name = country_row["country_name"] if country_row else "Unknown"
    ...
    for glazing in ["normal", "solar_control", "cdte_pv"]:
        annual_kwh, peak_kw, _ = office_cooling_from_epw(
            epw_path=epw_path,
            lat=req.lat,  # only affects internal geometry
    ...
    savings = {
        "solar_control": {
            "energy_percent": round(
                100 * (base["annual_cooling_kwh"] - results["solar_control"]["annual_cooling_kwh"])
                / base["annual_cooling_kwh"], 1
            ),
            "cost_usd": round(
                base["annual_cost_usd"] - results["solar_control"]["annual_cost_usd"], 2
            ),
            "carbon_kg": round(
                base["annual_carbon_kg"] - results["solar_control"]["annual_carbon_kg"], 1
            )
        },
        "cdte_pv": {
            "energy_percent": round(
                100 * (base["annual_cooling_kwh"] - results["cdte_pv"]["annual_cooling_kwh"])
                / base["annual_cooling_kwh"], 1
            ),
            "cost_usd": round(
                base["annual_cost_usd"] - results["cdte_pv"]["annual_cost_usd"], 2
            ),
            "carbon_kg": round(
                base["annual_carbon_kg"] - results["cdte_pv"]["annual_carbon_kg"], 1
            )
        }
    }
